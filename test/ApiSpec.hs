  ...
  where
    ...
    bidAcceptedJsonValue = object [
        ...
        "bid" .= object [
            ...
            "amount" .= Number 11,
            "at" .= String "2018-08-04T00:00:00Z" ] ]
    addAuctionOk = postWithHeader "/auctions" [(xJwtPayload, seller1), acceptJson, contentTypeJson] firstAuctionReqJson `shouldRespondWith` fromValue auctionAddedJsonValue
    addBidOk = postWithHeader "/auctions/1/bids" [(xJwtPayload, buyer1), acceptJson, contentTypeJson] "{\"amount\":11}" `shouldRespondWith` fromValue bidAcceptedJsonValue

    addAuctionSpec = describe "add auction" $ do
      it "possible to add auction" addAuctionOk
      it "not possible to same auction twice" $ do addAuctionOk; postWithHeader "/auctions" [(xJwtPayload, seller1), acceptJson, contentTypeJson] firstAuctionReqJson `shouldRespondWith` "AuctionAlreadyExists 1" {matchStatus = 400}
    ...
    addBidSpec = describe "add bids to auction" $ do
      ...
      it "not possible to add bid to non existant auction" $ postWithHeader "/auctions/2/bids" [(xJwtPayload, buyer1), acceptJson, contentTypeJson] "{\"amount\":10}" `shouldRespondWith` "Auction not found" {matchStatus = 404}
